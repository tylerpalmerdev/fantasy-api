-- NOTE: CONVERT ALL BOOLS TO TEXT FOR GOOGLE CSV --
SELECT s.pts as actual_stat, pl.bref_id,
...
-- GAME DATE & TIME --
g.game_time_24_et, t1.tz_hrs_over_utc as game_tz, g.day_of_week, (g.game_date - '2016-10-01') as days_into_season,
-- GAME LAT & LNG
...
JOIN (
    ...
    ((DATE_PART('year', '2016-11-15'::date) - DATE_PART('year', pl.dob::date)) * 12 +
    (DATE_PART('month', '2016-11-15'::date) - DATE_PART('month', pl.dob::date))) AS age,
    ((DATE_PART('year', '2016-11-15'::date) - DATE_PART('year', pl.debut_date::date)) * 12 +
    (DATE_PART('month', '2016-11-15'::date) - DATE_PART('month', pl.debut_date::date))) AS exp_months
    FROM nba_players pl
    LEFT OUTER JOIN (
    SELECT DISTINCT ON (player_id) *
    FROM nba_player_changelog
    WHERE changed_on BETWEEN '2016-10-01' AND (DATE '2016-11-15' + interval '1' DAY)
...
JOIN (
    ...
    ON g.game_id = (
        ...
        WHERE
            ...
            AND gp.game_date < '2016-11-15' AND gp.game_date > '2016-10-01'
...
JOIN (
    ...
    WHERE game_date < '2016-11-15' AND game_date > '2016-10-01' AND attendance IS NOT NULL
...
JOIN (
    WITH games_won AS (
        ...
        FROM nba_games
        WHERE game_date < '2016-11-15' AND game_date > '2016-10-01'
        GROUP BY team_id
    )
...
JOIN (
    SELECT player_id, (ARRAY_AGG(game_id))[1] as last_game_played_id, (ARRAY_AGG(team_won))[1] as won_last_game_played,
        ARRAY_AGG(pts) as recent_of_stat, ARRAY_AGG(mins) as recent_mins,
    ...
    FROM (
        SELECT CAST (row_num AS INTEGER ) AS row_num, player_id, pts, ROUND(CAST(mins AS numeric), 2) as mins, team_id, game_id, team_won, tz, lat, lng, game_date, game_time
        ...
        FROM (
            ...
            JOIN nba_teams t
                ON t.team_id = g.home_team_id
            WHERE g.game_date < '2016-11-15' AND g.game_date > '2016-10-01' -- SUB THIS OUT FOR DATE IN QUERY TEMPLATE STR
